; =================================================
; crash.asm ── 第26章の事件を、再現する
;
;   タスク0が、配列の大きさを取り違えて、
;   隣のタスク1のスタックを踏み潰す。
;
;   タスク0に悪意はない。タスク1に落ち度もない。
;   それでもタスク1は死ぬ。そして誰も、止めない。
;
;   これが「特権はあるのに、メモリの境界がない」ということ。
;
;   ビルド： vasmm68k_mot -Fbin -o crash.bin crash.asm
; =================================================
...
; =================================================
    ...
    dc.b    "MEIJIN CRASH DEMO                               "   ; $120 国内名(48)
    dc.b    "MEIJIN CRASH DEMO                               "   ; $150 海外名(48)
...
; --- タスク0：悪意はない。ただ、番地を間違えた ---
;     Buffer は256バイトしかないのに、4000バイト書き込む。
;     よくあるバグ。配列の大きさを取り違えただけ。
Buffer      equ     KStackBase  ; ★取り違えたポインタ。OSの保存領域を指している
...
.loop:
    ...
    ; --- ここが事件：カーネルの保存領域(＝各タスクの「世界」)を薙ぎ払う ---
    lea     Buffer,a0
    move.w  #256-1,d0           ; 256ワード。隣タスクのカーネルスタックまで届く
.fill:
    move.w  #$0000,(a0)+        ; ゼロで埋めていく
    dbra    d0,.fill            ; …$FF0200(隣の世界) を、軽々と越えていく

    trap    #0                  ; 譲る（隣人に、走る機会を与える）
    bra.s   .loop

; --- タスク1：被害者。何も悪いことはしていない ---
;     ただ、ふつうに数えているだけ。
;     しかし、眠っている間に「世界」（カーネルスタックに封印された
;     15本のレジスタ＋PC＋SR）を、隣人にゼロで塗り潰される。
TaskCounter:
    moveq   #0,d2
.loop:
    addq.w  #1,d2

    bsr     .report             ; ← ふつうの関数呼び出し
                                ;   戻り先がユーザスタックに積まれる
