; =================================================
; meijin10.asm ── MeijinOS68k v1.0（完成版）
;
;   特権の境界 ＋ 本物のシステムコール ＋ 例外の捕捉
;
;   タスク0: 時計（1秒に一回数える）
;   タスク1: カウンタ（Aボタンを押した回数）
;   タスク2: ゾンビ（譲らない行儀の悪い住人）
;
;   三人ともユーザモードで動く。カーネルには手が出せない。
;
;   ビルド： vasmm68k_mot -Fbin -o meijin10.bin meijin10.asm
; =================================================
...
; =================================================
    ...
    dc.b    "MEIJIN OS68K V1.0                               "   ; $120 国内名(48)
    dc.b    "MEIJIN OS68K V1.0                               "   ; $150 海外名(48)
...
SysGetInput:
    ori.w   #$0700,sr           ; 割り込みを止める（読む→空にする を不可分に）
    move.w  InputEdge,d0        ; メールボックスを読んで
    clr.w   InputEdge           ; 空にする
    rte                         ; rte が SR ごと戻すので、ここで開く
...
ReadPad:
    ...
    nop                         ; ポートが落ち着くのを待つ
    nop
    move.b  PAD_DATA,d0         ; Start(b7) A(b6) C B 右 左 下 上
    and.w   #$00FF,d0           ; 1バイトぶんだけ見る（bit7も残す）
    not.w   d0                  ; 押下=1 に反転（アクティブ・ロー）
    and.w   #$00FF,d0           ; 反転で立った上位を落とす

    move.w  PrevPad,d1
    not.w   d1
    and.w   d0,d1               ; エッジ = 今回 ∧ ¬前回

    ; --- OS専用の呼び鈴を、アプリに配る前に横取りする ---
    move.w  d1,d2
    and.w   #$C0,d2             ; Start(bit7) と A(bit6) の同時押しか？
    cmp.w   #$C0,d2
    bne.s   .notmagic

    bsr     KillZombie          ; OS専用の仕事
    and.w   #$3F,d1             ; この2ビットは、アプリに配らない
...
; =================================================
; KillZombie ── ホットキーで呼ばれる、OS専用の仕事
;   タスク2（ゾンビ）を殺す。
;
;   殺すとは、生死フラグを0にすること。それだけ。
;   コードもスタックも残る。失われるのは、選ばれる資格だけ。
; =================================================
KillZombie:
    movem.l d0/a0,-(sp)
