; =================================================
; preempt.asm ── MeijinOS68k v0.2（プリエンプティブ・マルチタスク）
;   タスクは譲らない。VBlank割り込みが、強制的に切り替える
;
;   ビルド： vasmm68k_mot -Fbin -o preempt.bin preempt.asm
;   実行  ： BlastEm に preempt.bin を放り込む
;
;   期待する結果：
;     どちらのタスクも trap #0 を呼ばないのに、両方とも増えていく
;     ＝ OSが時間を「取り上げて」配り直している証拠
;
;   v0.1 との差分は、たった二箇所：
;     ① ベクタ表30番（VBlank）を VBlankPreempt に向ける
;     ② タスクから trap #0 を消す
;   心臓部（Yield）は、一文字も変わらない
;
...
; =================================================
    ...
    dc.l    VBlankPreempt       ; 30: レベル6（VBlank）← 心臓部を直結
...
; =================================================
    ...
    dc.b    "MEIJIN OS68K V0.2                               "   ; $120 国内名(48)
    dc.b    "MEIJIN OS68K V0.2                               "   ; $150 海外名(48)
...
; =================================================
; VBlankPreempt ── 時間の支配者
;   定例業務（時刻・画面）を済ませてから、心臓部へ飛ぶ。
;   Yield は rte で帰るので、ここは bra でよい（戻ってこない）。
; =================================================
VBlankPreempt:
    movem.l d0-d1,-(sp)
...
;
;   どちらも行儀が悪い。誰も譲らない。
;   それでも両方が動くのは、OSが取り上げるから。
; =================================================
TaskA:
    addq.w  #1,CounterA         ; 自分の仕事をひたすら続ける
    bra.s   TaskA               ; もう、譲らない
